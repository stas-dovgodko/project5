...
class XMLReaderIterator implements Iterator, XMLReaderAggregate
{
    ...
    public function moveToNextSiblingElement()
    {
        ...
        while (self::valid()) {
            if ($this->reader->nodeType === XMLReader::ELEMENT && $this->reader->depth === $depth) {
                break;
            }
            self::next();
        }
...
class XMLElementXpathFilter extends XMLReaderFilterBase
{
    ...
    public function __construct(XMLReaderIterator $iterator, $expression)
    {
        parent::__construct($iterator);
