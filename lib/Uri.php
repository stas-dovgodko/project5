...
class Uri implements UriInterface
{
    ...
    public function resolve(Uri $uri)
    {
        ...
        return (isset($resolve[$hash = get_class($uri) . get_class($this) . $this->hashCode() . $uri->hashCode()])) ? $resolve[$hash] : ($resolve[$hash] = $this->createResolve($uri));
    }
    ...
    public function __toString()
    {
        ...
        return (string)((isset($str[$hash = $this->hashCode()])) ? $str[$hash] : ($str[$hash] = $this->buildStr()));
    }
    ...
    protected function buildStr()
    {
        ...
        if ($this->isOpaque()) {
            ...
            if ($this->scheme) $uri .= '//';
            $uri .= $this->buildAuthorityFragment();
    ...
    private function buildAuthorityFragment()
    {
        if ($this->host !== null) {
            $authority = '';
    ...
    public function withUserInfo($user, $password = null)
    {
        ...
        $url->authority = $url->buildAuthorityFragment();
